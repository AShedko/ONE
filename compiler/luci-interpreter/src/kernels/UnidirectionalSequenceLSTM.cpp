...
namespace luci_interpreter
{
    ...
    UnidirectionalSequenceLSTM::UnidirectionalSequenceLSTM(std::vector<std::pair<const Tensor *, int32_t>> &&inputs, std::vector<std::pair<Tensor *, int32_t>> &&outputs): Kernel(std::move(inputs), std::move(outputs))
    {
      // Do nothing
    }

//    void UnidirectionalSequenceLSTM::checkInputTensorDimensions(int n_input, int n_output,
//                                                                int n_cell, bool is_integer) const
//    {
...
//          is_integer ? DataType::S16 : input_to_forget_weights()->element_type();
//        LUCI_INTERPRETER_CHECK(cell_to_forget_weights()->element_type() ==
//                               cell_forget_weights_type);
//      }
//
...
//          is_integer ? DataType::S16 : input_to_forget_weights()->element_type();
//        LUCI_INTERPRETER_CHECK(cell_to_output_weights()->element_type() ==
//                               cell_output_weights_type);
//      }
//
...
//         (cell_to_forget_weights() != nullptr) && (cell_to_output_weights() != nullptr)) ||
...
//    }
    ...
    void UnidirectionalSequenceLSTM::configure(luci::CircleReader *circle_reader, int32_t index)
    {
      ...
      const auto input_to_output_weights_shape = luci::wrap(circle_reader->tensors()[input_to_output_weights_ind()]->shape());
...
//      }
      ...
      auto time_major = options->time_major;

      const auto n_batch = time_major ? input_tensor_shape[1] : input_tensor_shape[0];//input_shape.dim(1) : input_shape.dim(0);
      const auto n_cell = input_to_output_weights_shape[0];
      ...
      uint32_t num_elements = n_batch;

      if (use_cifg)
      {
        // Reserving space for Cell, Forget, Output gates
        //scratch_buffer_shape.dim(1) = n_cell * 3;
        num_elements = n_cell * 3;
      }
      else
      {
        // Reserving space for Input, Cell, Forget, Output gates
        //scratch_buffer_shape.dim(1) = n_cell * 4;
        num_elements = n_cell * 4;
      }

      // TODO: change to input->element_type
      const auto dtype = loco::DataType::FLOAT32;
      auto size = num_elements * getDataTypeSize(dtype);

      scratch_buffer()->set_alloc_size(size);
      scratch_buffer()->set_allocatable(true);
...
//      }
    ...
    void UnidirectionalSequenceLSTM::execute(luci::CircleReader *circle_reader, int32_t index) const
    {
      auto input_to_output_weights_dtype = luci::luci_datatype(circle_reader->tensors()[input_to_output_weights_ind()]->type());
      switch (input_to_output_weights_dtype)
      {
...
//    void UnidirectionalSequenceLSTM::evalHybrid(luci::CircleReader *circle_reader, int32_t index) const
//    {
//      const auto input_shape = input()->shape();
...
//            aux_input_size, n_output, output_batch_leading_dim, input_gate_scratch,
...
//            auto output_state_ptr = const_cast<float *>(getTensorData<float>(output_state())) +
//                                    b * output_batch_leading_dim;
//            auto cell_state_ptr =
//              const_cast<float *>(getTensorData<float>(cell_state())) + b * n_cell;
//
...
//    }
    ...
    void UnidirectionalSequenceLSTM::evalFloat(luci::CircleReader *circle_reader, int32_t index) const
    {
      circle::OperatorT oper_t;
      circle_reader->operators()[index]->UnPackTo(&oper_t);
      const auto *options = oper_t.builtin_options.AsUnidirectionalSequenceLSTMOptions();
      ...
      const auto input_tensor_shape = luci::wrap(circle_reader->tensors()[input_ind()]->shape());
      const auto input_to_output_weights_shape = luci::wrap(circle_reader->tensors()[input_to_output_weights_ind()]->shape());
      const auto recurrent_to_output_weights_shape = luci::wrap(circle_reader->tensors()[recurrent_to_output_weights_ind()]->shape());

      int max_time, n_batch;
      if (input_tensor_shape.size() == 3)
      {
        max_time = (time_major) ? input_tensor_shape[0] : input_tensor_shape[1];
        n_batch = (time_major) ? input_tensor_shape[1] : input_tensor_shape[0];
      }
      else
      {
        max_time = 1;
        n_batch = input_tensor_shape[0];
      }
      const auto n_input = input_tensor_shape[input_tensor_shape.size() - 1];
      const auto aux_input_size = 0;

      // n_cell and n_output will be the same size when there is no projection.
      const auto n_cell = input_to_output_weights_shape[0];
      const auto n_output = recurrent_to_output_weights_shape[1];
      ...
      TfLiteLSTMParams lstm_params;
      lstm_params.activation = get_tflite_activation(luci::luci_actfunc(options->fused_activation_function));
      lstm_params.cell_clip = options->cell_clip;
      lstm_params.proj_clip = options->proj_clip;
      lstm_params.asymmetric_quantize_inputs = options->asymmetric_quantize_inputs;

      const auto output_shape = luci::wrap(circle_reader->tensors()[output_ind()]->shape());
      const auto output_batch_leading_dim = output_shape[output_shape.size() - 1];
      if (time_major)
      {
        ...
        for (int32_t b = 0; b < n_batch; b++)
        {
          ...
          for (int32_t t = 0; t < max_time; t++)
          {
            ...
            eval_lstm::LstmStepFloat(
              ...
              getTensorData<float>(cell_gate_bias()), getTensorData<float>(output_gate_bias()),
              getTensorData<float>(projection_weights()), getTensorData<float>(projection_bias()),
              const_cast<const TfLiteLSTMParams *>(&lstm_params), /*n_batch=*/n_batch, n_cell,
              n_input, aux_input_size, n_output, output_batch_leading_dim, output_state_ptr,
              cell_state_ptr, input_gate_scratch, forget_gate_scratch, cell_gate_scratch,
