...
Fill::Fill(std::vector<std::pair<const Tensor *, int32_t>> &&inputs, std::vector<std::pair<Tensor *, int32_t>> &&outputs)
...
void Fill::configure(luci::CircleReader *circle_reader, int32_t index)
{
...
void Fill::execute(luci::CircleReader *circle_reader, int32_t index) const
{
  auto output_dtype = luci::luci_datatype(circle_reader->tensors()[output_ind()]->type());

  const auto dims_tensor_shape = luci::wrap(circle_reader->tensors()[dims_ind()]->shape());
  tflite::RuntimeShape dims_runtime_shape(dims_tensor_shape.size());
  for (int i = 0; i < dims_tensor_shape.size(); ++i)
  {
    dims_runtime_shape.SetDim(i, dims_tensor_shape[i]);
  }
  ...
  const auto value_tensor_shape = luci::wrap(circle_reader->tensors()[value_ind()]->shape());
  tflite::RuntimeShape value_runtime_shape(value_tensor_shape.size());
  for (int i = 0; i < value_tensor_shape.size(); ++i)
  {
    value_runtime_shape.SetDim(i, value_tensor_shape[i]);
  }

  switch (output_dtype)
  {
//    case DataType::S8:
//      tflite::reference_ops::Fill(value_runtime_shape, getTensorData<int8_t>(value()),
//                                  output_runtime_shape, getTensorData<int8_t>(output()));
//      break;
//    case DataType::S16:
...
//      break;
    case DataType::S32:
      tflite::reference_ops::Fill(value_runtime_shape, getTensorData<int32_t>(value()),
                                  output_runtime_shape, getTensorData<int32_t>(output()));
      break;
    case DataType::S64:
      tflite::reference_ops::Fill(value_runtime_shape, getTensorData<int64_t>(value()),
                                  output_runtime_shape, getTensorData<int64_t>(output()));
      break;
    case DataType::FLOAT32:
      tflite::reference_ops::Fill(value_runtime_shape, getTensorData<float>(value()),
