...
Add::Add(std::vector<std::pair<const Tensor *, int32_t>> &&inputs, std::vector<std::pair<Tensor *, int32_t>> &&outputs)
  : Kernel(std::move(inputs), std::move(outputs))
{
}

void Add::configure(luci::CircleReader *circle_reader, int32_t index)
{
...
void Add::execute(luci::CircleReader *circle_reader, int32_t index) const
{
...
void Add::evalFloat(luci::CircleReader *circle_reader, int32_t index) const
{
...
//  calculateActivationRange(luci::luci_actfunc(options->fused_activation_function), &activation_min, &activation_max);
  ...
  {
    tflite::reference_ops::Add(params, input_runtime_shape_1, getTensorData<float>(input1()),
